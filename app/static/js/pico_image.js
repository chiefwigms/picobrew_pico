...
// Load New Image - currently hardcoded for new_pico_recipe.html (recipe_image*)
function load_new_image(e){
    var reader = new FileReader();
    reader.onload = function(event){
      var img = new Image();
      img.onload = function(){
          var canvas = document.getElementById('recipe_image');
          var ctx = canvas.getContext('2d');
          ...
          var invert = (document.getElementById("recipe_image_invert").checked == true);
          var threshold = parseFloat(document.getElementById("recipe_image_threshold").value);
          var byte = 0;
          var byte_cnt = 0;
          recipe_img = '';
          for (var i = 0; i < img_bw.data.length; i += 4)
          {
            ...
            if (byte_cnt == 8)
            {
              ...
              recipe_img += ((tmp.length == 1) ? ('0' + tmp) : tmp); // %02x
              byte = 0;
              byte_cnt = 0;
            }
          }
